...
def main():
    ...
    for epoch in range(epochs):
        ...
        # 计算训练指标
        train_loss = train_loss_sum / train_tokens if train_tokens > 0 else 0
        ...
        if (epoch + 1) % log_interval == 0 or epoch == 0:
            print(f'Epoch {epoch + 1:4d} | '
                  ...
                  f'Val Loss: {val_loss:.4f} | Val PPL: {val_ppl:8.2f}')
            
            # 生成一个样本
            model.eval()
            with torch.no_grad():
                start_tokens = torch.tensor([[vocab['<bos>']]]).to(device)
                generated = model.generate(start_tokens, max_new_tokens=30, temperature=0.8)
                generated_text = ''.join([
                    id2token.get(idx, '') 
                    for idx in generated[0].tolist() 
                    if id2token.get(idx, '') not in ['<bos>', '<eos>', '<pad>', '<unk>']
                ])
                print(f'  生成样本: {generated_text}')
